...
module.exports = class $fireflyioUiMonitoring
{
  ...
  _renderTemplateRoute(ctx) {
    if (this.options.accessToken && ctx.url.indexOf('?access_token=' + this.options.accessToken) === -1) {
      ctx.status = 403;
      ctx.body = {
  ...
  _renderJSONRoute(ctx) {
    ...
    const servers_list = this.monitoring.modules.server.retrieveServersList()
      .map(server => ({
        url: server.url,
        total_clients: server.socket.getNumberClients()
      }));
  ...
  _initializeHttpRoute()
  {
    ...
    this.fireflyio.use((ctx, next) => {
      if (ctx.url.indexOf(this.options.route + '/json') === -1) return next();
      this._renderJSONRoute(ctx);
    });
    this.fireflyio.use((ctx, next) => {
      if (ctx.url.indexOf(this.options.route) === -1) return next();
      this._renderTemplateRoute(ctx);
    });
  }
};
